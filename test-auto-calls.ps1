...
try {
    ...
    if ($stats.data.students -and $stats.data.students.Count -gt 0) {
        ...
        Write-Host "   ℹ️  No students currently need follow-up" -ForegroundColor Yellow
        Write-Host ""
    }
} catch {
...
try {
    ...
    if ($result.data.details -and $result.data.details.Count -gt 0) {
        ...
        foreach ($detail in $result.data.details) {
            ...
            $statusIcon = if ($detail.status -eq "success") { "✅" } else { "❌" }
    ...
    Write-Host "❌ Failed to process follow-ups" -ForegroundColor Red
